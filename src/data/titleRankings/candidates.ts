...
function fromBatterLine(line: Extract<PlayerSeasonLine, { role: "batter" }>): TitleCandidate {
  ...
  return {
    ...
    available: {
      avg: d.avg != null,
      h: Number.isFinite(c.h),
      hr: Number.isFinite(c.hr),
      rbi: Number.isFinite(c.rbi),
      obp: d.obp != null,
      doubles: Number.isFinite(c.doubles),
      triples: Number.isFinite(c.triples),
      r: c.r != null && Number.isFinite(c.r),
      sb: c.sb != null && Number.isFinite(c.sb),
      bb: Number.isFinite(c.bb),
      // 犠打0も有効値。未入力(null/undefined)のみ未収録
      sac: c.sac != null && Number.isFinite(c.sac),
      ops: d.ops != null,
      risp: hasRisp,
